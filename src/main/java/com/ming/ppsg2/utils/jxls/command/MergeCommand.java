package com.ming.ppsg2.utils.jxls.command;
...
public class MergeCommand extends AbstractCommand {
	...
	protected Size poiMerge(CellRef cellRef, Context context, PoiTransformer transformer, int rows, int cols){
		...
		sheet.addMergedRegion(region);//poi api合并单元格
		...
		if(cellStyle == null){
			...
			//获取命令开始的行，解决未生成行后再合并空指针的问题 
			PoiCellData cellData = (PoiCellData)transformer.getCellData(this.getAreaList().get(0).getStartCellRef());
